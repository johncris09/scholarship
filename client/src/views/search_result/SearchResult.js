...
import { Box, IconButton, Skeleton, Button, Tooltip } from '@mui/material'
...
const SearchResult = ({ cardTitle }) => {
  ...
  const fetchApplicationDetails = (_id) => {
    setFetchApplicationDetailsLoading(true)
    api
      .get('applicant_details/' + _id)
      .then((response) => {
    ...
    await api
      ...
      .then((response) => {
        applicantDetailsForm.setValues({
          ...
          lastname: toSentenceCase(response.data[0].lastname),
          firstname: toSentenceCase(response.data[0].firstname),
          middlename: toSentenceCase(response.data[0].middlename),
          suffix: response.data[0].suffix,
          barangay: response.data[0].address,
          address: response.data[0].barangay_id,
          birthdate: response.data[0].birthdate,
          civil_status: toSentenceCase(response.data[0].civil_status),
          sex: response.data[0].sex,
          contact_number: response.data[0].contact_number,
          email_address: response.data[0].email_address,
          father_name: toSentenceCase(response.data[0].father_name),
          father_occupation: toSentenceCase(response.data[0].father_occupation),
          mother_name: toSentenceCase(response.data[0].mother_name),
          mother_occupation: toSentenceCase(response.data[0].mother_occupation),
        })
        fetchApplicationDetails(response.data[0].id)
      })
      .catch((error) => {
        console.info(error)
        // toast.error(handleError(error))
